<div class="row">
    <div class="title-wrap col-lg-12">
        <h3 class="title-left">門禁計費方式修改</h3>
    </div>
</div>
...
<form class="form-horizontal" action="<?php echo Yii::app()->createUrl('door/update'); ?>" method="post"
    ...
    <div class="panel panel-default">
        <div class="panel-body">
                ...
                <div class="col-sm-5">
                    <label class="radio-inline">
                        <input type="radio" name="status"
                               value="0" <?php echo ($model->status == 0) ? 'checked="checked"' : '' ?>>開啟
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="status"
                               value="1" <?php echo ($model->status == 1) ? 'checked="checked"' : '' ?>>關閉
                    </label>
                </div>
            </div>
            ...
            <div class="form-group">
                <label class="col-sm-2 control-label">建檔人:</label>
                <div class="col-sm-5">
                    <select class="form-control" name="builder">
                        <?php foreach($accounts as $key=>$value): ?>
                            <?php if($model->builder == $key):?>
                                <option selected="selected" readonly="readonly" value="<?= $value->id ?>">
                                    <?= $value->account_name  ?>
                                </option>
                            <?php else: ?>
