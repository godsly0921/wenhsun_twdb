<div class="row">
    <div class="title-wrap col-lg-12">
        <h3 class="title-left">首頁廣告圖管理列表</h3>
       <!-- --><?php /*if ($canCreate === true): */?>
            <a href="<?php echo Yii::app()->createUrl('website/ad_new'); ?>" class="btn btn-success btn-right">新增首頁廣告圖</a>
       <!-- --><?php /*endif;*/?>
    </div>
</div>
        ...
        <table id="specialcaseTable" width="100%" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <?php foreach($ad_data as $key => $value){ ?>
                <tr class="gradeC" role="row">
                    ...
                    <td><img src="<?=DOMAIN. "image_storage/O/" .$value['single_id'] . ".jpg"?>" width="100%"></td>
                    <td><?=$value['sort']?></td>              
                    <td>
                        <a class="oprate-right" href="<?php echo Yii::app()->createUrl('website/ad_update/') ?>/<?= $value['single_id'] ?>">
...
<script>
    $(".oprate-del").on('click', function(){
        ...
        if (answer == true) {
            ...
            form.setAttribute('action', "<?php echo Yii::app()->createUrl('website/ad_delete') ?>");
