                ...
                if($diffYear < 1){
                    if ($dDiff->format('%m') < 6) {
                        ...
                        if($diffEndTime->format('%m') >= 6 || $diffEndTime->format('%y')> 0){
                            ...
                            $special_leave = round((3 + 
                                round(
                                    $this->leaveMap[1]-
                                    ($seniority_month+(($onBoardDate->format('d')-1)/$month_day))/12 * $this->leaveMap[1],2))
                                * 8 * 60);
                    ...
                    if ($dDiff->format('%m') >= 6) {
                        ...
                        // 到職日當年滿 半年的特休
                        $special_leave = round((3 - round(($seniority_month+(($onBoardDate->format('d')-1)/$month_day))/6 * 3,2))  * 8 * 60);
                        ...
                        // 到職日滿 半年未滿 1 年的特休
                        $special_leave = round((
                            round(($seniority_month + ($onBoardDate->format('d')-1)/$month_day)/6 * 3,2) + 
                            round($this->leaveMap[1]-(($seniority_month+($onBoardDate->format('d')-1)/$month_day)/12 * $this->leaveMap[1]),2))
                             * 8 * 60);
                        $this->create_Specialleaveyear($employee, $now->format('Y')."-01-01", $now->format('Y')."-12-31", ($dDiff->format('%y')*12 + $dDiff->format('%m')),  $special_leave);
                    }
                }else{
                    ...
                    if (isset($this->leaveMap[$diffYear])) {
                        $seniority_month = $onBoardDate->format('m')-1;
                        // var_dump(round(
                        //     round(($seniority_month + ($onBoardDate->format('d')-1)/$month_day)/12 * $this->leaveMap[$diffYear],2) + 
                        //     round($this->leaveMap[($diffYear+1)]-(($seniority_month+($onBoardDate->format('d')-1)/$month_day)/12 * $this->leaveMap[($diffYear+1)]),2)
                        //     ,1));
                        // exit();
                        if($diffYear < 25){
                            $special_leave = round((
                            ...
                            * 8 * 60);
                            ...
                            $special_leave = round((
                            round(($seniority_month + ($onBoardDate->format('d')-1)/$month_day)/12 * $this->leaveMap[$diffYear],2) + 
                            round($this->leaveMap[25]-(($seniority_month+($onBoardDate->format('d')-1)/$month_day)/12 * $this->leaveMap[25]),2))
                            * 8 * 60);
                        }
                        ...
                        $special_leave = round((
                            round(($seniority_month->format('m') + ($onBoardDate->format('d')-1)/$month_day)/12 * 30,2) + 
                            round(30-(($seniority_month->format('m')+($onBoardDate->format('d')-1)/$month_day)/12 * 30),2))
                            * 8 * 60) ;
                        $this->create_Specialleaveyear($employee, $now->format('Y')."-01-01", $now->format('Y')."-12-31", ($dDiff->format('%y')*12 + $dDiff->format('%m')),  $special_leave);
                    }
                }
