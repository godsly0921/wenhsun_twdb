...
Yii::import('luckywave.widgets.CBaseListView');
Yii::import('luckywave.widgets.grid.CDataColumn');
Yii::import('luckywave.widgets.grid.CLinkColumn');
Yii::import('luckywave.widgets.grid.CButtonColumn');
Yii::import('luckywave.widgets.grid.CCheckBoxColumn');
...
class CGridView extends CBaseListView
{
	...
	public function init()
	{
		...
		if(empty($this->updateSelector))
			throw new CException(Yii::t('luckywave','The property updateSelector should be defined.'));
		if(empty($this->filterSelector))
			throw new CException(Yii::t('luckywave','The property filterSelector should be defined.'));

		if(!isset($this->htmlOptions['class']))
			$this->htmlOptions['class']='grid-view';

		if($this->baseScriptUrl===null)
			$this->baseScriptUrl=Yii::app()->getAssetManager()->publish(Yii::getPathOfAlias('luckywave.widgets.assets')).'/gridview';
	...
	protected function createDataColumn($text)
	{
		if(!preg_match('/^([\w\.]+)(:(\w*))?(:(.*))?$/',$text,$matches))
			throw new CException(Yii::t('luckywave','The column must be specified in the format of "Name:Type:Label", where "Type" and "Label" are optional.'));
